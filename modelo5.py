...
data_augmentation_path = os.path.join(data_dir, 'data_augmentation.npy')
metadata_path = os.path.join(data_dir, 'metadata.npy')

data_augmentation = np.load(data_augmentation_path)
metadata = np.load(metadata_path, allow_pickle=True)

print(f'La dimensión de data_augmentation es: {data_augmentation.shape}')
print(f'La dimensión de metadata es: {metadata.shape}')
print(metadata)
metadata_repetida = np.concatenate([metadata, metadata])  # Duplica metadata para que coincida con el tamaño

print(f'La nueva dimensión de metadata_repetida es: {metadata_repetida.shape}')

# Generar etiquetas (por simplicidad, asumamos que cada archivo tiene una etiqueta basada en el gesto)
labels = np.array([item['gesture'] for item in metadata_repetida])

# Filtrar solo las etiquetas de interés: 13, 15 y 16
#valid_classes = [13, 15, 16]
#mask = np.isin(labels, valid_classes)

#data_augmentation = data_augmentation[mask]
#labels = labels[mask]
...
X_train, X_val, y_train, y_val = train_test_split(data_augmentation, labels, test_size=0.2, random_state=42)
...
# Redimensionar datos para que se ajusten
...
model_checkpoint = ModelCheckpoint(filepath='model_bestVIII.keras', 
...
model_path = 'model_bestVIII.keras'
...
plt.tight_layout()
plt.savefig('curvas_entrenamientoVIII.png')
plt.show()
